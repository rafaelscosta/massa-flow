import { users, clients, appointments, findUserById, findClientById, updateAppointment, addCommunicationLog } from './db.js'; // Added addCommunicationLog
import fs from 'fs';
import path from 'path';
import { trackEvent } from './analytics.js'; // Import analytics tracker
...
try {
    ...
    console.error("Failed to load practice templates. Ensure JSON files are correct in data/practiceTemplates:", error);
    // Fallback to empty objects to prevent crashes if files are missing during certain test runs
    practiceTemplates = { autonomo: { routines: {} }, clinica: { routines: {} }, spa: { routines: {} } };
}


// Utility function for message variable injection
function formatMessage(templateString, variables) {
    if (!templateString) return "Error: Message template is undefined.";
